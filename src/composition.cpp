...
bool available(const CompiledNode& node, const glayout::ResolvedNode& geometry,
               const Host& host) {
...
void box(std::vector<PaintCommand>& output, PaintStratum stratum, NodeIndex node,
    ...
    output.push_back(PaintCommand{PaintKind::Box, stratum, node, rect, clip, style,
                                  {}, {}, {}, 0.0, ImageMode::Stretch,
                                  {255, 255, 255, 255}, 1.0f, 8.0f});
}

void text(std::vector<PaintCommand>& output, PaintStratum stratum, NodeIndex node,
          glayout::Rect rect, glayout::Rect clip, const BoxStyle& style,
          TextStyle text_style, std::string value) {
    output.push_back(PaintCommand{PaintKind::Text, stratum, node, rect, clip, style,
                                  std::move(text_style), std::move(value), {}, 0.0,
                                  ImageMode::Stretch, {255, 255, 255, 255}, 1.0f, 8.0f});
}
...
void image(std::vector<PaintCommand>& output, PaintStratum stratum, NodeIndex node,
    ...
    output.push_back(PaintCommand{PaintKind::Image, stratum, node, rect, clip, {}, {}, {},
                                  part.asset, 0.0, part.image_mode, part.tint, part.opacity,
                                  part.slice});
}
...
void visual_part(std::vector<PaintCommand>& output, const CompiledNode& node,
                 WidgetPart part, PresentationState state, PaintStratum stratum,
                 NodeIndex index, glayout::Rect rect, glayout::Rect clip,
                 const BoxStyle& fallback) {
...
void text_part(std::vector<PaintCommand>& output, const CompiledNode& node,
               WidgetPart part, PresentationState state, PaintStratum stratum,
               NodeIndex index, glayout::Rect rect, glayout::Rect clip,
               const BoxStyle& fallback, TextStyle text_style, std::string value) {
    const PartPresentation* presentation = part_for(node, part, state);
    const BoxStyle style = presentation && presentation->override_box ? presentation->box : fallback;
...
void compose_control(std::vector<PaintCommand>& output, const CompiledNode& node,
                     const NodeState& state, NodeIndex index, bool focused,
                     glayout::Rect content, glayout::Rect clip, const Value& bound) {
    const BoxStyle& style = resolve_style(node.source, state, focused);
    const PresentationState visual_state = presentation_state(
        node.source, state.hovered, state.pressed, state.open, focused, bound);
    const double range = node.source.maximum - node.source.minimum;
    const double ratio = range > 0.0
                             ? (numeric_value(bound, node.source.minimum) - node.source.minimum) /
                                   range
                             : 0.0;
    ...
    if (node.source.control == ControlKind::Slider) {
        visual_part(output, node, WidgetPart::Track, visual_state, node.source.stratum,
                    index, geometry.track, clip, track_style(style));
        visual_part(output, node, WidgetPart::Fill, visual_state, node.source.stratum,
                    index, geometry.fill, clip, fill_style(style));
        ...
        visual_part(output, node, WidgetPart::Thumb, visual_state, node.source.stratum,
                    index, geometry.thumb, clip, thumb);
        ...
        text_part(output, node, WidgetPart::CurrentValue, visual_state, node.source.stratum,
                  index, geometry.value, clip, style, value_style, value);
        return;
    }
    ...
    if (node.source.control == ControlKind::Toggle) {
        ...
        visual_part(output, node, WidgetPart::Indicator, visual_state,
                    node.source.stratum, index, geometry.indicator, clip, indicator);
        return;
    }
    ...
    if (node.source.control == ControlKind::Select) {
        ...
        const PartPresentation* indicator =
            part_for(node, WidgetPart::Indicator, visual_state);
    ...
    if (node.source.control == ControlKind::TextInput && state.editing)
        ...
        rendered = value;
    text(output, node.source.stratum, index, geometry.label, clip, style,
         node.source.text_style, std::move(rendered));
}
...
void compose_popup(std::vector<PaintCommand>& output, const CompiledNode& node,
    ...
    const PresentationState visual_state = presentation_state(
        node.source, state.hovered, state.pressed, state.open, false, {});
    const BoxStyle frame = popup_style(node.source.style.normal);
    visual_part(output, node, WidgetPart::Popup, visual_state, PaintStratum::Overlay,
                index, geometry.frame, viewport, frame);
    for (std::size_t row = 0; row < geometry.options.size(); ++row) {
        ...
        text(output, PaintStratum::Overlay, index, label, viewport, style,
             node.source.text_style, node.source.options[option].label);
    }
}
...
void compose(const CompiledView& view, const glayout::GraphRuntime& layout,
    ...
    const glayout::Rect viewport = layout.nodes().empty()
                                       ? glayout::Rect{}
                                       : layout.nodes().front().clip;
    for (NodeIndex index = 0; index < view.nodes.size(); ++index) {
        ...
        const Value bound = host.read && !node.source.binding.empty()
                                ? host.read(node.source.binding)
                                : Value{};
        const BoxStyle& style = resolve_style(node.source, state[index], focus == index);
        const PresentationState visual_state = presentation_state(
            node.source, state[index].hovered, state[index].pressed, state[index].open,
            focus == index, bound);
        visual_part(output, node, WidgetPart::Frame, visual_state, node.source.stratum,
                    index, border, resolved.clip, style);
        if (node.source.control != ControlKind::None &&
            ...
            output.push_back(PaintCommand{kind, node.source.stratum, index, content,
                                          resolved.clip, style, node.source.text_style,
                                          node.source.text, node.source.asset,
                                          numeric_value(bound), ImageMode::Stretch,
                                          {255, 255, 255, 255}, 1.0f, 8.0f});
        }
        compose_popup(output, node, state[index], index, border, viewport);
    }
    std::stable_sort(output.begin(), output.end(), [](const PaintCommand& left,
