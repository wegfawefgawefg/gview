...
void Runtime::frame(const glayout::ResolveInput& resolution, const InputFrame& input, Host& host) {
    ...
    const auto move_focus = [&](NodeIndex target, bool restore_group_memory = true) {
        if (target == invalid_node || target >= view_.nodes.size() || !available(target)) return;
        const CompiledFocusGroup* previous_group = focus_group_for(view_, focus_);
        const CompiledFocusGroup* target_group = focus_group_for(view_, target);
        const bool crosses_group = previous_group != target_group;
        if (crosses_group && previous_group && previous_group->remember && focus_ != invalid_node)
            remembered_focus_[previous_group->id] = view_.nodes[focus_].source.layout_id;
        if (crosses_group && restore_group_memory && target_group && target_group->remember &&
            target_group->entry == target) {
            const auto remembered = remembered_focus_.find(target_group->id);
            if (remembered != remembered_focus_.end()) {
                const auto found = view_.indices.find(remembered->second);
                if (found != view_.indices.end() && available(found->second)) target = found->second;
            }
        }
        focus_ = target;
    };
    ...
    if (input.pointer.moved || input.pointer.pressed || input.pointer.released) {
        ...
        for (NodeIndex index = 0; index < view_.nodes.size(); ++index) {
            ...
            if (!state_[index].open || node.source.control != ControlKind::Select ||
                ...
                continue;
            const glayout::Rect anchor = detail::displayed_rect(
                view_, state_, layout_.nodes()[node.layout_index].border, node.layout_index);
            const PopupGeometry popup = resolve_popup_geometry(
                node.source, anchor, layout_.nodes().front().clip, state_[index].pending_option);
            const auto row = std::find_if(popup.options.begin(), popup.options.end(),
                                          [&](glayout::Rect option) {
                                              return contains(option, input.pointer.x,
                                                              input.pointer.y);
                                          });
            if (row != popup.options.end()) {
                state_[index].pending_option =
                    popup.first_option + static_cast<std::size_t>(row - popup.options.begin());
        ...
        for (std::size_t reverse = view_.nodes.size();
            ...
            const glayout::Rect border =
                detail::displayed_rect(view_, state_, geometry.border, node.layout_index);
    ...
    if ((input.pointer.pressed || input.pointer.moved) && focus_ != invalid_node &&
        ...
        const glayout::Rect content = detail::displayed_rect(
            view_, state_, layout_.nodes()[slider.layout_index].content, slider.layout_index);
        const Value bound = host.read && !slider.source.binding.empty()
                                ? host.read(slider.source.binding)
                                : Value{};
        const double range = slider.source.maximum - slider.source.minimum;
        const double ratio = range > 0.0
                                 ? (numeric_value(bound, slider.source.minimum) -
                                    slider.source.minimum) /
                                       range
                                 : 0.0;
        const WidgetGeometry widget = resolve_widget_geometry(slider.source, content, ratio);
        set_slider_from_pointer(slider.source, widget.track, input.pointer.x, host);
        paint_dirty_ = true;
    }
    ...
    for (NavAction action : input.navigation) {
        ...
        move_focus(next_focus(view_, layout_.nodes(), focus_, action, available));
        if (focus_ != previous) {
            ensure_focus_visible(view_, layout_, state_, focus_);
    ...
    if (paint_dirty_) {
        detail::compose(view_, layout_, state_, focus_, host, paint_);
        ++stats_.paint_builds;
        paint_dirty_ = false;
    }
}
